...
def home(request):
    ...
    vehicle = request.GET.get("type")
    query = request.GET.get("q")

    products = Product.objects.all()

    if vehicle == "car":
        products = products.filter(category="car")

    elif vehicle == "bike":
        products = products.filter(category="bike")
...
def search(request):
    ...
    products = Product.objects.filter(part__icontains=query)[:6]
    ...
    for p in products:
        data.append({
            ...
            "brand": p.brand,
            "price": p.price,
            "image": p.image.url if p.image else "/static/noimage.png"
        })
